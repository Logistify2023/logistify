...
import { TypesCreateComponent } from './create/create.component';
import { TypesDetailsComponent } from './details/details.component';
import { TypesUpdateComponent } from './update/update.component';
import { TypesRestoreComponent } from './restore/restore.component';
import { TypesDeleteComponent } from './delete/delete.component';
...
export class TypesComponent implements OnInit {
  ...
  public displayedColumns : string[]  = ['customer_type', 'status', 'created_at', 'updated_at', 'details', 'update', 'restore', 'delete'];
  ...
  loadGridData() {
    this.loading = true;
    this.apiCustomer.getClientsTypes(0, false, false, this.dataDeleted).subscribe((e: any) => {
      this.loading = false;
      if (e.result) {
  ...
  openModalDetails(id: number) {
    const dialogRef = this.dialog.open(TypesDetailsComponent, {
      width: '80vw',
  ...
  openModalRestore(id: number) {
    const dialogRef = this.dialog.open(TypesRestoreComponent, {
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  openModalDelete(id: number) {
    const dialogRef = this.dialog.open(TypesDeleteComponent, {
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
    dialogRef.afterClosed().subscribe(result => {
