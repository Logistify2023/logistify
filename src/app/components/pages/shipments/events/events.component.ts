...
import { EventsCreateComponent } from './create/create.component';
import { EventsDetailsComponent } from './details/details.component';
import { EventsUpdateComponent } from './update/update.component';
import { EventsRestoreComponent } from './restore/restore.component';
import { EventsDeleteComponent } from './delete/delete.component';
...
export class EventsComponent implements OnInit {
  ...
  public displayedColumns : string[]  = ['key', 'event', 'status', 'created_at', 'updated_at', 'details', 'update', 'restore', 'delete'];
  ...
  loadGridData() {
    this.loading = true;
    this.apiShipments.getEvents(0, false, this.dataDeleted).subscribe((e: any) => {
      this.loading = false;
      if (e.result) {
  ...
  openModalDetails(id: number) {
    const dialogRef = this.dialog.open(EventsDetailsComponent, {
      width: '80vw',
  ...
  openModalCreate() {
    const dialogRef = this.dialog.open(EventsCreateComponent, {
      disableClose: true
    });
  ...
  openModalUpdate(id: number) {
    const dialogRef = this.dialog.open(EventsUpdateComponent, {
      width: '80vw',
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
  ...
  openModalRestore(id: number) {
    const dialogRef = this.dialog.open(EventsRestoreComponent, {
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  openModalDelete(id: number) {
    const dialogRef = this.dialog.open(EventsDeleteComponent, {
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
    dialogRef.afterClosed().subscribe(result => {
