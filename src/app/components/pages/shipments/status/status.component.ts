...
import { StatusUpdateComponent } from './update/update.component';
import { StatusRestoreComponent } from './restore/restore.component';
...
export class StatusComponent implements OnInit {
  ...
  public displayedColumns : string[]  = ['state', 'type', 'status', 'created_at', 'updated_at', 'details', 'update', 'restore', 'delete'];
  ...
  loadGridData() {
    this.loading = true;
    this.apiShipments.getStatus(0, false, this.dataDeleted).subscribe((e: any) => {
      this.loading = false;
      if (e.result) {
  ...
  openModalCreate() {
    const dialogRef = this.dialog.open(StatusCreateComponent, {
      disableClose: true
    });
  ...
  openModalRestore(id: number) {
    const dialogRef = this.dialog.open(StatusRestoreComponent, {
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  openModalDelete(id: number) {
    const dialogRef = this.dialog.open(StatusDeleteComponent, {
      data: { _id: id, dataDeleted: this.dataDeleted },
      disableClose: true
    });
    dialogRef.afterClosed().subscribe(result => {
